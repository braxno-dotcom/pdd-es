...
(function () {
  ...
  // ⚠️ Десять из двенадцати — с картинкой (картинок теперь хватает: 54 вопроса). Борис поспорил, что знаки выпадают
  // не в каждом третьем вопросе, и был прав: при прежних долях их было пять-шесть
  // на билет. Двенадцать знаковых вопросов — верх той самой вилки 8-12, так что
  // правдоподобия это не ломает, а знак теперь и правда каждый третий.
  var SENALES_CON_DIBUJO = 10;
  var CUOTAS = { senales: 12, velocidad: 3, prioridad: 3, seguridad: 5,
  ...
  function pintar() {
    // ⚠️ ПЕРЕВОД НЕ ПОЯВЛЯЕТСЯ САМ. Каждый новый вопрос приходит по-испански, и
    // русский открывается только нажатием кнопки — на ЭТОТ вопрос. Я дважды пробовал
    // иначе (сперва перевод всегда, потом липкий тумблер как у французов), и оба раза
    // получалось одно: человек читает перевод, а испанскую строку не видит вовсе.
    // На экзамене её не будет, поэтому сначала пробуешь понять сам.
    conRuso = false;
    aplicarRuso();
    mostrarBoton(true);
